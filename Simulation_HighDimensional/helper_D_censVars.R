...
# fx to extract SE from Cox model (unadjusted)
calc.beta.se <- function(times, status, data)
{
  # Fit model:
  Cox <- coxph(Surv(times, status) ~ D, data = data)
  # Return SE (on log scale):
  return(summary(Cox)$coef[,3])  # column 3 is "se(coef)"
}
...
# fx to extract SE from IPW-weighted Cox model
calc.beta.se.IPW <- function(times, status, IPW.weights, data)
{
  # Fit model:
  Cox <- coxph(Surv(times, status) ~ as.factor(D),
               data = data, weights = IPW.weights)
...
# fx to extract SE from IPCW-weighted Cox model
calc.beta.se.IPCW <- function(Tstart, Tstop, status, IPCW.weights, data.long)
{
  # Fit model:
  Cox <- coxph(Surv(Tstart, Tstop, status,
                    type = "counting") ~ D,
...
# fx to extract SE from combo-weighted Cox model
calc.beta.se.comb <- function(Tstart, Tstop, status, comb.weights, data.long)
{
  # Fit model:
  Cox <- coxph(Surv(Tstart, Tstop, status,
                    type = "counting") ~ D,
